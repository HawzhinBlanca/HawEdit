...
| `proposals.py` | Phase 3 | The `hawedit-revise` CLI plus Phase 3's two mutating capabilities: propose a boundary or caption revision, validate each against the real render-gate check (`assert_boundary_invariant`/`assert_captions_within_clip` — Kurdish invariants #2/#4), commit only after an attributed, explicit "yes" on a real terminal, then render a real second deliverable set (MP4/ASS/SRT/EDL) from `PipelineRun.selected_sentences` — the original render and delivery untouched. `revisions/<id>.json` records carry `"kind"` so each render function refuses the other's record. No `pydantic_ai` import at module level, so `--help` and every function here need none of it. |
