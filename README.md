...
| `workflow_control.py` | Phase 3 | `start_pipeline` (D-A19): `propose_start_pipeline`/`commit_start_pipeline`, the propose/commit split for actually running a durable pipeline. Scoped to three parameters (`source`, `media_id`, `transcript`), not `pipeline.py`'s full 25-flag CLI — compliance flags and the QC-bypass flag stay outside agent reach. Refuses to overwrite a `work_dir` that already has a completed run. Records a `DecisionDelta` (`kind="start_pipeline"`) on every outcome. The `hawedit-workflow` CLI. |
| `workflow_agent.py` | Phase 3 | The agent wrapper around `workflow_control.py`: registers only `propose_start_pipeline_tool`. `commit_start_pipeline` does not appear anywhere in this module's source, checked by an AST scan. Found and fixed a real Policy Gate false positive along the way: `_FORBIDDEN_NAME_FRAGMENTS`'s bare `"pip"` matched "pipeline" — narrowed to `"pip_"` (`policy.py`) rather than renaming this tool away from the architecture record's own vocabulary. |
