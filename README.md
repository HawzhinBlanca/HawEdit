...
| `proposals.py` | Phase 3 | The `hawedit-revise` CLI plus Phase 3's first mutating capability: propose a boundary revision, validate it against the real `assert_boundary_invariant` (Kurdish invariant #2), commit only after an attributed, explicit "yes" on a real terminal. No `pydantic_ai` import — the CLI's `--help` and `commit_boundary_revision` need none of it. |
