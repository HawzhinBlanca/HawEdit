...
|---|---|---|---|---|---|
| **T3.1** | **P1** | **Two-pass linear loudnorm.** First pass measures (`print_format=json`), second applies `measured_*` with `linear=true`. Record both passes in the contract. | `render.py:147-150` dynamic mode; LRA 2.2 LU on s25-25. | A: `test_loudnorm_runs_linear_with_measured_inputs`. B: `ebur128` on the re-render: I within ±0.5 LU, TP ≤ −1, LRA recorded; media-tier test on speech, not a sine. C: T1.2. | — |
| **T3.2** | **P2** | **Speech chain: `afftdn` denoise, gentle `deesser`, presence EQ.** ffmpeg-native, no dependency; parameters measured, not chosen: SNR before/after on three ep29 spans. ADR for the chain. | Nothing between ASR-grade audio and delivery. | A: filter-graph tests. B: SNR table. **D: Hawa listens to before/after.** | Hawa (taste); ADR |
...
| **T4.4** | **P2** | **Hook taxonomy, payoff and loop scoring.** Verdict gains `hook_type ∈ {question, claim, contrast, story_open, confession}`, `payoff_strength`, `ends_on_a_beat`; keep `reason_ckb`. Used as tiebreakers only until labels exist. | `payoff_at_ms` is never used; `reason_ckb` is discarded. | A: schema tests. B: distribution on ep29 candidates recorded as judgment. D: correlation with H2 labels later. | — |
...
| **T4.8** | **P2** | **Boundary extension that completes a sentence.** When a shot-cut or turn extension pulls in unselected words, allow it if and only if those words complete a sentence; refuse otherwise (as now). Measure how often extension applies before/after on ep29 candidates. | `pipeline.py:2181-2199` refuses nearly always in continuous speech. | A: invariant-preserving tests. B: extension rate table. | — |
