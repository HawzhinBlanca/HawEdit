...
| **T0.4** | **DONE** — **Single-writer rule for the checkout.** Recorded single-writer checkout policy in `AGENTS.md`, bound in `test_single_writer_checkout_policy_is_bound_in_agents_md`. Canonical source video rule preserved at `.agents/rules/canonical-source-video.md` with SHA `47235f42...` for Zar #29 chunk. | Two agents wrote to one tree; `BLOCKED.md` #12 was this. | A: `test_claims` binds the rule text. | **Hawa** |
...
| **T1.4** | **DONE** — **Real-media test tier.** (`specs/real-media-tier/ledger.log`). Created `tests/media/` with `provenance.json`, fail-not-skip invariant in `tests/conftest.py`, 8 media-tier tests in `tests/media/test_real_media_tier.py` passing on `ep29-chunk50min.mp4` in 35.2s, bound by `test_real_media_tier_fixture_provenance_and_binding_is_recorded` (Level A) and `evidence/real-media-tier.md` (Level B). | §7.1–7.2: nothing asserts anything about a real face, real speech loudness, or real cut placement. | A: the tier's own tests. E: required check at the SHA. Depends on T0.3. | Hawa (runner) |
...
| **T2.12** | **P2** | **DONE** — Two-person layout (`specs/two-person-layout/ledger.log`). When diarization shows a fast exchange (turns < 3.0 s) inside a clip with >= 2 speakers, creates a top/bottom vertical split (`Reframe.TWO_PERSON_SPLIT` / `two_person_split_filter`) of both tracked faces instead of whip-pans or rapid cuts. Supported via `--two-person-split {auto,always,never}` and pipeline integration. | Needs T2.1. Podcast dialogue is the owner's format. | A: layout tests (`test_two_person_split_*`). B: gate green (3,540 passed); ledger updated with cryptographic evidence. | — |
| **T2.13** | **P3** | **DONE** — Brand kit: lower-third, logo, end card, progress bar (`specs/brand-kit/ledger.log`). Speaker name and bio from per-episode metadata (`SpeakerBio`) + diarization label rendered as lower-third tags via libass with zero subtitle collision; logo watermark overlay with strict fail-stop on missing file; optional animated progress bar (`ProgressBarConfig`); 2 s branded end card (`EndCardConfig`). ADR D-268. | A team always does this; the spec never asked. | A: `test_brand_kit_*`, `test_build_ass_with_speaker_metadata_and_turns`, `test_render_clip_*logo*`, `test_pipeline_parser_brand_kit_flags`. B: gate green (3,556 passed); ledger updated with cryptographic evidence. | — |
...
| **T4.2** | **P1** | **DONE** — Path B as independent discovery (`specs/independent-path-b/ledger.log`). Stop seeding Path B with Path A's rank-1 slice. Run retrieval over all planned windows against a fixed Sorani query set for non-verbal beats (`DEFAULT_NONVERBAL_VISUAL_QUERY`: "پێکەنین، کاردانەوە، سەرسوڕمان، جووڵە") via `--visual-nonverbal` / `visual_nonverbal=True`, within the 8-frame ceiling; union with Path A per §3. Record per-path candidate counts and "found by B only". ADR D-271. | `pipeline.py:1351-1376`: today B resembles A. §3: *neither path filters the other*. | A: `test_path_b_runs_without_a_path_a_seed`, `test_auto_select_accepts_visual_alone_using_default_nonverbal_query`. B: gate green (3,571 passed); ledger updated with cryptographic evidence. | — |
