...
> Source: `ZAR38MinTest.mp4` â€” h264 640Ã—360, 25 fps, AAC 44.1 kHz stereo, **2313.8 s**.

D-134 got the OmniASR runtime provisioned, which let Stage 1 run on real Kurdish for the first time.
...
emitted 15 tokens for it â€” more text than the audio can carry. Refusing to align that is invariant #5
working as designed.
...
One raise inside a generator expression discarded a finished Stage 0 â€” 74 MB of extracted audio, a
51 MB proxy, 547 cut regions â€” plus every other region's GPU inference. **6 of 547** regions sit in
...
generator existed in both and D-134 had just finished paying for duplicated invocation logic. A region
...
`from_json`, and a pre-D-135 transcript with no such key still reads (`.get`, because refusing to read
...
â€” without it, "report every region as unaligned" satisfies every other test here and would silently
...
Gate: `VERIFY OK â€” 1197 passed, 0 skipped`.
