# A 30-line fake `pytest` printed VERIFY OK â€” and poisoned the committed floor on its way out

> Measured 2026-08-09 on hawapc01 against `64f440c`, against a green 1,155 baseline.

D-104 closed the case where `PY` was not an interpreter that runs this project. It could not close
...
==> test evidence test evidence OK â€” 1200 collected, 1200 passed, 0 skipped
VERIFY OK â€” hawedit gate green
exit=0 elapsed=4s
...
Four seconds, exit 0, zero test bodies executed. Lint, typecheck and format ran for real â€” only the
...
written during this run, by the thing pretending to be pytest. Layer 3's own reasoning â€” "exit code
is not evidence; the report is" â€” holds only while the report comes from pytest.

**The damage outlives the run.** `write_floor` ratcheted the committed floor to 1,200, so every
honest run afterwards would have been refused for a bar a forgery invented â€” a fake green that leaves
...
Enumerating ways to redirect an import â€” `PYTHONPATH`, user site-packages, a directory in the working
tree â€” is the same losing shape as the blacklist of no-op commands `gate.py`'s docstring already
...
`hawedit` deliberately does **not** â€” it is installed editable in both places, so its file lives in
...
Folded into D-104's existing probe rather than added beside it: one call, one refusal path, before any
step runs.

```
  PYTHONPATH=<fake pytest>  -> exit 3, names `pytest -> â€¦\fakepytest\pytest\__init__.py`
                               floor unchanged at 1155
  PY=/usr/bin/true.exe      -> exit 3, unchanged from D-104
...
CAUGHT   verify.sh stops asking about provenance (D-104 only)            FAILED=1
...
* **The last mutation** â€” reverting `verify.sh` to D-104's import-only probe â€” is caught by exactly
...
test ratcheted the floor 1,161 â†’ 1,200, which the "floor unchanged" assertion caught and which was
...
Gate: `VERIFY OK â€” 1161 passed, 0 skipped`.
