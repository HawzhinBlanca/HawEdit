...
D-084 pinned one constant to its record â€” `MINIMUM_HOURS` against D-009 â€” after measuring that 3.0
...
## Measured â€” what notices a drift, and why
...
DEFAULT_DISAGREEMENT_CER 0.15 -> 0.25          RED, 2 tests â€” behaviour only
MINIMUM_HOURS             3.0 -> 1.0           RED, 1 test â€” the record pin D-084 added
```

Two of the four are completely unprotected, and each is instructive for a different reason.

**`NVENC_MIN_FRAME â†’ (64, 64)` is exactly the value D-045 records as the historical defect** â€” the
probe size that made `encoder_available` report a working NVENC unavailable on the one machine Â§6 puts
...
which holds at 1080Ã—1920 against any small pair. The relation is the right assertion, and it cannot
pin the recorded measurement. Stated plainly: drifting this constant has no behavioural effect today,
because the probe runs at the output size regardless â€” what would rot is D-045's recorded measurement
of what NVENC actually refuses, which is the number a future reader would reach for.

**`DEFAULT_PAUSE_MS â†’ 800` was invisible for a reason worth naming.** Every pause test passes
...
coverage and measures nothing â€” the same shape as D-126's substring assertion and D-127's `skipped=0`
...
(D-009, D-014, D-015, D-045), so D-130 restates three more in canonical form, quoting the entries that
gave them in prose only â€” `MATERIAL_GAIN_RATIO = 0.10` (D-010's "â‰¥10% relative reduction"),
`DEFAULT_IOU_MATCH = 0.5` (D-020's "IoU â‰¥ 0.5"), `RETRIEVE_K = 50` (D-102's "Â§3's depth of 50") â€” plus
...
* D-130's own prose spelled the placeholder in the real form, so the check read its own documentation
...
CAUGHT   MATERIAL_GAIN_RATIO drifts from D-130's record           FAILED=2
...
works â€” the two together are the pair that matters, and the repo's standard is single-mutation
auditing, so this is recorded as a neutral survivor in D-078's sense rather than as protection.

`NVENC_MIN_FRAME` is caught by exactly one test â€” the new record check â€” which is the whole point:
nothing else in the suite can see that number change.

Gate: `VERIFY OK â€” 1172 passed, 0 skipped`.
