...
D-135 got Stage 1 through the whole file. This iteration pointed the visual path at it â€” Qwen
embedding and reranking, VideoChat3-4B, TimeLens2, all present on this machine â€” and it stopped after
**three** windows.

## Measured

```
âœ— zar38final:s2:w0 planned 36 frames over 17720 ms and ffmpeg produced 34. One frame of tail
...
35 is one short of the plan â€” exactly the "tail rounding" the same message calls normal. Then this
...
**Scope:** any window with an even plan and an odd delivery. `ceil(duration Ã— 2.0)` is even about
half the time, so about half of all windows â€” which is why the run got through three.
...
audit pins it â€” widening to 2 is caught by that control alone.
...
second iteration running where the audit's real catch was my own new guard (D-135's blank reason).

**`len(extracted) > window.frame_count` had no test at all** â€” Â§3 Stage 2's 64-frame ceiling was
...
Qwen, loaded VideoChat3-4B â€” and then hit a real resource wall:
...
That is not a capacity fact, it is a **divergence from frozen Â§6**. BLUEPRINT lines 346â€“347:
...
VideoChat3 reader, so all three land on GPU 0 while GPU 1 held 1.3 GiB. Â§6 puts the embedder and
...
Gate: `VERIFY OK â€” 1202 passed, 0 skipped`.
