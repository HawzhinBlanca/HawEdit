# §6 spreads the video phase across two GPUs; the code ran it on one

> Measured 2026-08-09 on hawapc01 (2× RTX 3090 Ti, 23.99 GiB each) against `099fa3c`.
> Source: `ZAR38MinTest.mp4` — 640×360 h264, 25 fps, 2313.8 s.

D-104 got the visual path from 3 windows to 164, and it then died on VideoChat3. BLUEPRINT §6 is
explicit and frozen:
...
before   GPU 0: 18.30 GiB allocated,  3.59 GiB free   (embedder + reranker + reader together)
         ...
         ✗ CUDA out of memory. Tried to allocate 21.83 GiB.

after    GPU 0: 10.44 GiB allocated, 12.18 GiB free   (the reader alone)
         ✗ CUDA out of memory. Tried to allocate 21.83 GiB.
```

**7.86 GiB freed on GPU 0**, Stage 2's indexing moved to GPU 1 — exactly what §6 asks for — and the
...
Path B reader on GPU 0. Both assignments come from §6, so nothing was chosen. `--timelens-device` was
already `cuda:1`.

`build_parser` was extracted from `main` so the defaults are assertable — a comment claiming §6 is not
§6 being followed, and the test now reads the parsed values.
...
the suite **green** — a default nothing reads is not an assignment, which is D-094's substring failure
...
§6 calls the reader "(segmented)", and I suspected the code pushed every window through one forward.
It does not — `read_scenes` is one call per window, docstring: *"One call per window rather than one
model batch: §3 calls segmentation mandatory"*. The 21.83 GiB is **one window**. Checking beat
...
weights: 32.3 GiB against a 23.99 GiB card. §3 Stage 2's 64-frame ceiling and this checkpoint's memory
appetite are in tension on a 3090 Ti — which is a fact about the pair, not a bug in either.
...
Gate: `VERIFY OK — 1206 passed, 0 skipped`.
