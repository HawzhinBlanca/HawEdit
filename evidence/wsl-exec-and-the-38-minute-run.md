...
> Source: `C:\Users\Wareen\Desktop\Test Videos\ZAR38MinTest.mp4` — h264 640×360, 25 fps,
...
38-minute Kurdish video, with no `--transcript` and no `--verdict` — the two flags whose own help
...
--omni-asr --gemini --visual --timelens --face-reframe --auto-select
    ✗ no Gemini API key. Run `python -m hawedit.credentials` … exit 2

--omni-asr --visual --timelens --face-reframe --auto-select
    ✗ canonical OmniASR WSL2 runtime is not provisioned. Run hawedit-asr-setup … exit 2
...
## Defect 1 — `wsl.exe --` ate the environment (fixed, D-102)
...
call passes `env PYTHONPATH=<source>` to reach `hawedit.asr_worker` — so Stage 1 would have died on
...
framing, and the model emitted 15 tokens for it — more text than the audio can carry, which is
...
reason and continues — but a transcript that silently omits a segment is worse than a refusal, so the
...
## Defect 2 (measured, not fixed) — the fingerprint invalidates itself

`package_fingerprint` hashes **every `*.py` in the package**, so any edit — including this change's
own lint fix — invalidates the provisioned runtime, and the pipeline says "not provisioned" again.
...
provisioned* from *provisioned, then the source changed* — the conflation D-099 fixed for
downloaded-versus-runnable.

Gate: `VERIFY OK — 1190 passed, 0 skipped`.
