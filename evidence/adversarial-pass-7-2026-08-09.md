# Adversarial pass #7 — invariants #1 and #3, against the real run's artifacts

> Run 2026-08-09 on hawapc01 against `771ca17`.
> Target: **M0.4**, DONE — "#1 enforced three ways (refuse-rewrite, frozen types, SHA-256 tamper
> evidence); #3 enforced by distinct types (mypy) + `assert_model_input` (runtime) + stale-norm
> detection. ASR provenance is validated against §7 at construction."

Every previous pass used fixtures. This one used the artifacts 38 minutes of Kurdish actually produced,
on a copy, so the originals were never modified — verified afterwards: the real file still matches its
sidecar.

## Part 1 — the invariants, on the real 820,835-byte transcript
...
## Part 2 — revert each mechanism, check the test goes red
...
RED    provenance: §7 role validation at construction                 (2 tests)
...
**It is not dead code.** It is the layer that matters when the sidecar is *gone* — the state someone
...
first layer. The messages differ — "already exists or is being written" versus "already exists." — so
...
`SyntaxError` rather than on a caught regression — D-082's wrong-reason catch, produced by the audit
...
than no audit, and this is the second harness defect this week (D-104's Windows file lock was the first).

Re-run after the fix: **5/5**, with the raw-file link caught by exactly the new test.

Gate: `VERIFY OK — 1209 passed, 0 skipped`.
