...
| **M5** | Stage 2 visual index + Stage 3 Path B | Visual recall | **WIP — Stage 2 is DONE end to end: contract, scene-window plan, and both real models running on hawapc01 (M5.1, M5.2). Stage 3 Path B's contract is DONE and the union runs two-sided given a reader; its model (M5.4) has weights on disk and is unstarted** |
...
and M6.3, the three model integrations. Their weights are **on disk** (33.7 GB across five §7
checkpoints) and the GPU stack is installed and measured, so what is left in each is the adapter
code (`evidence/gpu-stack.md`, D-048). **Not** M0.16: it was
...
| M5.4 | Real `MCG-NJU/VideoChat3-4B` behind the Path B contract, and the SV6D prompt | TODO | Weights local (8.4 GB, Apache-2.0, 4.86B params) and the GPU stack is verified, but the premise check found the path is **not** clear: the model loads in 4.8 s with `lm_head.weight` **absent from the checkpoint and randomly initialised** — the projection from hidden states to tokens, so Path B would emit SV6D labels that looked like labels. Its config contradicts itself (`tie_word_embeddings` False at the top level, True in `text_config`) and transformers 5.14.1 resolves it the wrong way; the checkpoint targets 4.57 and its own demos do a plain load, so the head is meant to be tied. `evidence/m5-4-videochat3-lm-head.md`, D-054. Unstarted; the adapter must tie the head **and verify the tie**, record the override of a third-party config, then still write the SV6D prompt and respect §3's 256-frame budget. |
...
| M6.3 | Real `MCG-NJU/TimeLens2-4B` behind the contract above | TODO | Weights local — 9.1 GB, Apache-2.0, 4437.8M params — and the GPU stack is verified (`evidence/gpu-stack.md`). The contract and its relevance gate are DONE (M6.1); this is the model behind them. Unstarted. |
