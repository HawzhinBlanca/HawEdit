...
| M5.2 | Real `Qwen3-VL-Embedding-2B` / `-Reranker-2B` behind the interfaces above | TODO | **Everything under it is in place and verified; the adapter itself is unwritten.** Weights local (4.0 GB each, Apache-2.0), GPU stack installed and measured — torch 2.13.0+cu130, both 3090 Ti doing bf16 work — and the embedder loads on `cuda:0` in 4.9 s at 3.98 GiB returning 2048-d L2-normalised vectors for real Kurdish text (`evidence/gpu-stack.md`). Two findings decide how it must be written, D-048: a window goes in as `{"video": frames}` (one embedding) and never as a list (four embeddings, then somebody takes a mean — which is §7's stated reason for excluding CLIP); and the loader the checkpoint itself declares, `sentence-transformers`, silently tells the model **fps=24** for frames that are 1 s apart and refuses `video_metadata`, so Stage 2 has to go through the `transformers` processor instead. |
