...
| M5.4 | Real `MCG-NJU/VideoChat3-4B` behind the Path B contract, and the SV6D prompt | TODO | Weights local — 8.4 GB, Apache-2.0, 4472.4M params — and the GPU stack is verified (`evidence/gpu-stack.md`). §3's 256-frame budget (~17.7 GB) fits one 24 GiB card and `path_b.py` already refuses past it. Unstarted; the SV6D prompt is still unwritten. Expect D-048's fps finding to apply here too — this model takes video the same way. |
