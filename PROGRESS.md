...
| M5.2 | Real `Qwen3-VL-Embedding-2B` / `-Reranker-2B` behind the interfaces above | PARTIAL | `src/hawedit/qwen_visual.py` + `tests/test_qwen_visual.py` (9 tests) and `src/hawedit/video_input.py` + `tests/test_video_input.py` (16 tests). **The embedder runs on real media**: three windows over the fixture, 2048-d, unit-norm vectors, index built in 10.4 s at 7.94 GiB on `cuda:0`, and a Kurdish query retrieves with dense 1-based ranks (`evidence/m5-2-embedder.md`). Pooling is read from the checkpoint and a `mean` recipe is refused; §7's role check runs before the 4 GB load; `cuda:0` on a machine without CUDA is refused rather than run on the CPU; invariant #3 is enforced inside `embed_text`. Getting here found two silent defects — a 4162 ms window reaching the model stamped 0.1 s (D-049) and a 1400 ms scene arriving as a single frame — both now caught. **Recorded as unexplained, not validated:** this route and the `sentence-transformers` loader the checkpoint declares differ by cosine 0.955 on identical text; four prompt placements do not close it, and the cause is not the pooling. D-050. **Shortfall:** the **reranker is unwritten** — `VisualReranker` is still an unimplemented Protocol, so §3 Stage 2's "top 50 → rerank → keep 5–10" is retrieval only, and `rerank_and_keep` has no producer. |
