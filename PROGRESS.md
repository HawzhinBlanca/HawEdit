...
| M5.4 | Real `MCG-NJU/VideoChat3-4B` behind the Path B contract, and the SV6D prompt | TODO | **The model now runs.** Premise verified end to end (D-055): on the pinned `transformers==4.57.6` with `vision_config.attn_impl="sdpa"` it loads with `missing_keys: NONE`, `lm_head` correctly tied, and describing a real frame of the fixture it answered *'A red number "0" is centered on a black background.'* — coherent, not a random head. Getting there found **three** ways `transformers` 5.14.1 broke it: a silently randomised `lm_head` (D-054), `KeyError: 'inputs_embeds'` in `prepare_inputs_for_generation`, and a vision tower calling `flash_attn` (which has no Windows wheels — `sdpa` is the setting). `evidence/m5-4-videochat3-lm-head.md`. Unstarted as an adapter: still to write are the `VideoUnderstanding` implementation, the SV6D prompt, §3's 256-frame budget, and D-049's `video_metadata` handling. |
