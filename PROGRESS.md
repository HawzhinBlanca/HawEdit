...
| M5.2 | Real `Qwen3-VL-Embedding-2B` / `-Reranker-2B` behind the interfaces above | PARTIAL | `src/hawedit/video_input.py` + `tests/test_video_input.py` (16 tests) — the input layer, built because a measurement found two ways to get it silently wrong (`evidence/m5-2-video-timestamps.md`, D-049). **A 4162 ms window at 1 fps reached the model stamped 0.0–0.1 seconds** — forty times compressed. An `fps` key in the content dict is accepted and ignored (`input_ids` byte-identical for 1, 24 and 2 fps); `video_metadata` in the content dict likewise; `sentence-transformers`, the loader the checkpoint itself declares, cannot pass it at all. Only a top-level `video_metadata=` works, giving (0.5, 2.5). The guard reads the **decoded prompt**, and the negative control is the real broken output. Second defect, in the code written for the first: the fixture's 1400 ms scenes yield **1 frame of 2 planned** — ffmpeg samples at interval centres — and a one-frame video block has no temporal structure at all, which is §7's reason for excluding CLIP from the other side. Refused now, with the remedy named. Weights local, both 3090 Ti verified, embedder measured at 4.23 GiB. **Shortfall:** the adapter itself is unwritten — nothing yet implements `VisualEmbedding` or the `VisualReranker` protocol, so §3 Stage 2 still cannot build an index. |
