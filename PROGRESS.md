...
| M1.4 | Canonical ASR worker, model readiness and reproducible WSL environment | PARTIAL | `src/hawedit/asr_worker.py`, `src/hawedit/models.py`, `src/hawedit/wsl_asr_locks.py`, and `src/hawedit/smoke.py`. Code and exact dependency/asset receipts are composed. **Shortfall:** this merged source still needs a fresh live WSL receipt and hardware acceptance. |
...
| M3.9 | WSL-ASR dependency vulnerability disposition gate | PARTIAL | `src/hawedit/vex.py`, `src/hawedit/wsl_audit_locks.py`, `src/hawedit/wsl_vex_gate.py`, `security/wsl-asr-vex.json` and their tests. The protected hardware job installs an exact 29-wheel scanner in an isolated WSL environment, requires pip-audit 2.10's exact object schema, revalidates the canonical receipt/assets, and evaluates captured policy bytes bound to the complete 140-distribution identity, lock digests, assets and HawEdit source digest. The console command ships and release smoke starts it. **Shortfall:** this merged source has no current canonical `.ready` receipt, so no live acceptance report exists yet; the policy source digest must be regenerated and reviewed after source stabilization before provisioning and the protected run. `evidence/wsl-asr-vex.md`, D-123. |
