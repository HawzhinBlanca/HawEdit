...
# --- D-190: the constants checked against the declarations they claim to come from ----------
...
def test_the_declared_rate_and_minimum_are_what_the_checkpoints_declare() -> None:
    """`DECLARED_SAMPLING_FPS` and `_MIN_SAMPLED_FRAMES` are stated in `visual_index.py` as the
    checkpoints' numbers rather than §3's. Assert that against the recorded declarations, not
    against the sentence that used to carry them."""
    from hawedit.visual_index import (
        _MIN_SAMPLED_FRAMES,
        DECLARED_SAMPLING_FPS,
        DECLARED_VIDEO_PREPROCESSORS,
    )

    rates = {c["fps"] for c in DECLARED_VIDEO_PREPROCESSORS.values()}
    minimums = {c["min_frames"] for c in DECLARED_VIDEO_PREPROCESSORS.values()}

    assert rates == {DECLARED_SAMPLING_FPS}, DECLARED_VIDEO_PREPROCESSORS
    assert minimums == {_MIN_SAMPLED_FRAMES}, DECLARED_VIDEO_PREPROCESSORS


def test_the_temporal_patch_constant_is_the_strictest_declared_not_a_shared_value() -> None:
    """The claim this pins was **false** until D-190: the source said all four checkpoints ship
    `temporal_patch_size: 2`. Measured, `MCG-NJU/VideoChat3-4B` ships **1** and the other three
    ship 2.

    The constant is still 2, for a reason the old sentence did not give: `extract_window_frames`
    extracts a window once and D-140's cache hands the same files to the embedder *and* the
    reader, so the count must satisfy the coarsest patch among them. This asserts that
    relationship — `max`, not agreement.
    """
    from hawedit.visual_index import DECLARED_VIDEO_PREPROCESSORS

    sizes = [c["temporal_patch_size"] for c in DECLARED_VIDEO_PREPROCESSORS.values()]

    assert max(sizes) == TEMPORAL_PATCH_FRAMES, (
        f"one extraction feeds every visual model, so the trim must satisfy the coarsest patch: "
        f"{DECLARED_VIDEO_PREPROCESSORS}"
    )
    # The control. If every checkpoint agreed, `max` would be indistinguishable from "what they
    # all declare" — which is precisely the claim that was wrong — and this test would pass while
    # measuring nothing.
    assert len(set(sizes)) > 1, (
        "the recorded checkpoints now agree on temporal_patch_size, so `max` no longer "
        "distinguishes the strictest from a shared value — re-read the configs and D-190"
    )


def test_every_visual_model_in_section_7_has_its_declarations_recorded() -> None:
    """The table is only worth asserting against if it covers the models it claims to. A §7
    visual model missing from it would leave its patch size out of the `max` above, which is the
    ...
    visual = {
        ...
        if getattr(entry, "role", None) in _VISUAL_ROLES
    }

    assert visual, "no §7 model carries a visual role; the roles above have drifted"
    assert visual == set(DECLARED_VIDEO_PREPROCESSORS), (
        f"§7 visual models and the recorded declarations disagree: "
        f"only in §7 {visual - set(DECLARED_VIDEO_PREPROCESSORS)}, "
        f"only recorded {set(DECLARED_VIDEO_PREPROCESSORS) - visual}"
    )
