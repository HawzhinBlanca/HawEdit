...
def describe_migration_path() -> str:
    ...
    return (
        ...
        "is the only place `@DBOS.step()`/`@DBOS.workflow()` appear. Two callers import it — "
        "`src/hawedit/durable.py` (`run_durable()`) and `src/hawedit/workflow_control.py` "
        "(`commit_start_pipeline`'s own call to `run_durable()`, D-A19) — and both defer the "
        "import inside a function body, past the point where argument parsing or proposal "
        "validation would already have failed or exited, so `--help` and an invalid/declined "
        "proposal never need `dbos` installed (D-A2/D-A3). Nothing in `agent.py`, "
        "`proposals.py`, `editor_agent.py`, `learning.py`, `promotion.py` or `workflow_agent.py` "
        "imports `durable_workflow.py` at all — verified by grep, not assumed. A Temporal "
        "migration therefore rewrites `durable_workflow.py` and updates its two deferred call "
        "sites, not every caller.\n\n"
